...
// Info: (20260114 - Luphia) Language type
export type Language = 'en' | 'zh-TW' | 'ko' | 'ja';
type Dictionary = typeof en;

// Info: (20260114 - Luphia) Helper to get nested value by key string "auth_modal.login_btn"
...
export function I18nProvider({ children }: { children: ReactNode }) {
  const [language, setLanguageState] = useState<Language>('zh-TW'); // Info: (20260114 - Antigravity) { Default to zh-TW }
  const [dictionary, setDictionary] = useState<Dictionary>(zhTw);

  useEffect(() => {
    // Info: (20260114 - Antigravity) { Load persisted language }
